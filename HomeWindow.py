...
class HomeScreen(QMainWindow):
    ...
    def addAddStudentPage(self):
        ...
        slot_id=QLabel("Slot ID : ")
        slot_id.setStyleSheet("color:#fff;padding:8px 0px;font-size:20px")
        error_label=QLabel("")
        ...
        slot_input=QLineEdit()
        slot_input.setStyleSheet("color:#fff;padding:8px 0px;font-size:20px")
        vtype=QComboBox()
        ...
        layout.addWidget(slot_id)
        layout.addWidget(slot_input)
        ...
        button.clicked.connect(lambda:self.addVehicles(name_input.text(),vehicle_input.text(),mobile_input.text(),slot_input.text(),vtype.currentIndex(),error_label))
        self.vertical_2.addWidget(frame)

    def addVehicles(self,name,vehicleno,mobile,slot_id,index,error_label):
        vtp=2
        if index==0:
            vtp=2
        else:
        ...
        data=self.dbOperation.AddVehicles(name,vehicleno,mobile,str(vtp), int(slot_id))
        if data==True:
